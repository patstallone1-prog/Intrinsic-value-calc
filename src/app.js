import { DEFAULT_INPUTS, FIXTURES, TAXONOMY, computeValuation, deriveContext, normalizeInputs } from "./valuationEngine.js"
...
function renderField(field) {
  ...
  return `
    <label class="field">
      <span>${field.label}</span>
      <input data-field="${field.key}" type="${field.type === "text" ? "text" : "number"}" value="${escapeHtml(inputDisplayValue(value, field.type))}" min="${min}" max="${max}" step="${step}" />
      <small>${formatValue(value, field.type)}</small>
    </label>
  `
}
...
function renderResults(result) {
  ...
  const valueDifference = marketValue > 0 ? result.outputs.fairCommonEquity / marketValue - 1 : null
  const trackRows = [
      ...
      <section class="panel hero-panel">
        <div class="valuation-comparison">
          ...
          <div class="market-value-block">
            <p class="eyebrow">${state.marketSnapshot?.tradingDays ? `${state.marketSnapshot.tradingDays}-day average market value` : "Observed market value"}</p>
          ...
          <div class="difference-block ${valueDifference !== null && valueDifference < 0 ? "difference-block--negative" : ""}">
            ...
            <small>${valueDifference === null ? "Load market data to compare" : valueDifference >= 0 ? "above recent market value" : "below recent market value"}</small>
          </div>
        </div>
        <div>
